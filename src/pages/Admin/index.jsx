...
function ContactsSection({ toast }) {
  ...
  const save = async (id, status, adminNotes) => {
    try { await contactApi.updateStatus(id, { status, adminNotes }); toast.success('✅ Mis à jour'); execute(); setSelected(null) }
    ...
    <div>
      <div className={styles.filters}>
        {['all','new','contacted','quoted','confirmed','cancelled','completed'].map(s => (
          <button key={s} className={`${styles.filterBtn} ${filter===s?styles.filterActive:''}`} onClick={() => setFilter(s)}>{s==='all'?'Tous':STATUS_LABELS[s]||s}</button>
        ))}
        <input className={styles.searchBox} placeholder="🔍 Rechercher..." value={search} onChange={e=>setSearch(e.target.value)} />
      </div>
      <div className={styles.card}>
        <div className={styles.cardHeader}><span className={styles.cardTitle}>📬 Réservations ({filtered.length})</span></div>
        <div style={{ overflowX:'auto' }}>
          <table className={styles.table}>
            <thead><tr><th>Client</th><th>Pack</th><th>Thème</th><th>Date</th><th>Pers.</th><th>Statut</th><th></th></tr></thead>
            <tbody>
              {filtered.map(c => (
...
function ContactModal({ c, onClose, onSave }) {
  ...
  return (
    <Modal isOpen title="📬 Réservation" onClose={onClose}
      footer={<><Button variant="ghost" onClick={onClose}>Fermer</Button><Button variant="primary" onClick={() => onSave(c.id,status,notes)}>💾 Sauvegarder</Button></>}>
      <div className={styles.detailGrid}>
        {[['Client',`${c.prenom} ${c.nom}`],['Email',c.email],['Téléphone',c.phone||'—'],['Pack',c.pack?.toUpperCase()],['Thème',c.theme],['Date',new Date(c.date).toLocaleDateString('fr-FR')],['Ville',c.ville||'—'],['Personnes',c.guests]].map(([l,v])=>(
          <div key={l} className={styles.detailItem}><div className={styles.detailLbl}>{l}</div><strong style={{ fontSize:'.88rem' }}>{v}</strong></div>
        ))}
      </div>
      {c.message && <p style={{ color:'var(--muted)', fontSize:'.85rem', lineHeight:1.6, padding:'10px', background:'rgba(255,255,255,.03)', borderRadius:8, marginBottom:'1rem' }}>{c.message}</p>}
      <ASelect label="Statut" value={status} onChange={setStatus} options={['new','contacted','quoted','confirmed','cancelled','completed'].map(s=>({v:s,l:STATUS_LABELS[s]||s}))} />
...
function ProductsSection({ toast }) {
  ...
  const { data, loading, execute } = useApi(() => productsApi.getAll({ limit:100 }), [], true)
  ...
  return (
    <div>
      <div className={styles.filters}>
        <button className={`${styles.filterBtn} ${filter==='all'?styles.filterActive:''}`} onClick={() => setFilter('all')}>Tous ({products.length})</button>
        {ALL_CATS.map(c => (
          <button key={c} className={`${styles.filterBtn} ${filter===c?styles.filterActive:''}`} onClick={() => setFilter(c)}>{c.charAt(0).toUpperCase()+c.slice(1)}</button>
        ))}
        <input className={styles.searchBox} placeholder="🔍 Rechercher..." value={search} onChange={e=>setSearch(e.target.value)} />
        <Button variant="primary" size="sm" style={{ marginLeft:'auto', whiteSpace:'nowrap' }} onClick={() => { setEditing(null); setModalOpen(true) }}>+ Ajouter</Button>
      </div>
      <div className={styles.card}>
        <div className={styles.cardHeader}><span className={styles.cardTitle}>📦 Produits ({filtered.length})</span></div>
        <div style={{ overflowX:'auto' }}>
          <table className={styles.table}>
            <thead><tr><th>Image</th><th>Produit</th><th>Fournisseur</th><th>Catégorie</th><th>Prix</th><th>Stock</th><th>Statut</th><th>Actions</th></tr></thead>
            <tbody>
              {filtered.map(p => (
                <tr key={p.id} className={styles.tr}>
                  <td>
                    {p.imageUrl
                      ? <img src={p.imageUrl.startsWith('/')?`${API_BASE}${p.imageUrl}`:p.imageUrl} alt={p.nameF} className={styles.productThumb} onError={e=>{e.target.style.display='none'}} />
                      : <div className={styles.productEmoji}>{p.emoji||'🎁'}</div>}
                  </td>
                  <td><strong>{p.nameF}</strong>{p.nameE&&<div style={{ fontSize:'.75rem', color:'var(--muted)' }}>{p.nameE}</div>}</td>
                  <td>
                    {p.supplier
                      ? <span style={{ fontSize:'.78rem', background:'rgba(34,197,94,.1)', color:'var(--green)', borderRadius:6, padding:'3px 8px' }}>🤝 {p.supplier.name}</span>
                      : <span style={{ fontSize:'.75rem', color:'var(--muted)' }}>Otaku Pulse</span>}
                  </td>
                  <td><Badge variant="gray" style={{ fontSize:'.65rem' }}>{p.category}</Badge></td>
                  <td>
                    <span style={{ fontFamily:'var(--font-title)', color:'var(--green)' }}>{p.price?.toLocaleString()} F</span>
                    {p.oldPrice&&<div style={{ fontSize:'.72rem', color:'var(--muted)', textDecoration:'line-through' }}>{p.oldPrice?.toLocaleString()}</div>}
                  </td>
      ...
      {modalOpen && <ProductModal product={editing} onClose={() => { setModalOpen(false); setEditing(null) }} onSave={save} toast={toast} />}
    </div>
  )
}
...
function ProductModal({ product:p, onClose, onSave, toast }) {
  const [form, setForm] = useState({
    nameF:p?.nameF||'', nameE:p?.nameE||'', slug:p?.slug||'', category:p?.category||'posters',
  ...
  const handleImageUpload = async (imageData, imageMime) => {
    if (!p?.id) { toast.info('Sauvegardez d\'abord le produit pour uploader une image.'); throw new Error('Pas encore créé') }
    setUploadingImg(true)
    try {
      await productsApi.uploadImage(p.id, imageData, imageMime)
      s('imageUrl', `${API_BASE}/api/upload/product/${p.id}/image`)
  ...
  return (
    <Modal isOpen title={p?'✏️ Modifier produit':'📦 Nouveau produit'} onClose={onClose} wide
      footer={<><Button variant="ghost" onClick={onClose}>Annuler</Button><Button variant="primary" onClick={() => onSave({ ...form, price:parseFloat(form.price)||0, oldPrice:parseFloat(form.oldPrice)||null, stock:parseInt(form.stock)||0, supplierId:form.supplierId||null, images:form.imageUrl?[form.imageUrl]:[] })}>💾 Enregistrer</Button></>}
    >
      ...
      <div style={{ marginBottom:'1rem' }}>
        <label style={{ display:'block', fontSize:'.7rem', fontWeight:700, letterSpacing:1, color:'var(--muted)', marginBottom:5, textTransform:'uppercase' }}>Fournisseur (vide = produit Otaku Pulse)</label>
        <select value={form.supplierId} onChange={e => { s('supplierId',e.target.value); s('isOwnProduct',!e.target.value) }}
          ...
          <option value="" style={{ background:'#0c1a2e' }}>⚡ Otaku Pulse (notre produit)</option>
          {suppliers.filter(s=>s.isActive).map(s => <option key={s.id} value={s.id} style={{ background:'#0c1a2e' }}>🤝 {s.name}</option>)}
        </select>
      </div>

      {/* Image upload */}
      <div style={{ marginBottom:'1rem' }}>
        <label style={{ display:'block', fontSize:'.7rem', fontWeight:700, letterSpacing:1, color:'var(--muted)', marginBottom:5, textTransform:'uppercase' }}>Image du produit</label>
        {p?.id ? (
          <ImageUploader
            currentUrl={form.imageUrl || null}
            onUpload={handleImageUpload}
            placeholder="📸 Uploader une image produit"
            maxSizeMB={5}
          />
        ) : (
          <div style={{ background:'rgba(255,255,255,.03)', border:'1px dashed rgba(255,255,255,.15)', borderRadius:10, padding:'1rem', textAlign:'center', color:'var(--muted)', fontSize:'.82rem' }}>
            📸 Sauvegardez d'abord le produit, puis uploadez l'image en cliquant sur ✏️ Modifier
          </div>
        )}
        <div style={{ marginTop:8 }}>
          <AInput label="Ou URL d'image externe" value={form.imageUrl} onChange={v=>s('imageUrl',v)} placeholder="https://...image.jpg" />
        </div>
      </div>

      <div className={styles.formGrid2}>
        <ASelect label="Catégorie *" value={form.category} onChange={v => s('category',v)} options={ALL_CATS.map(c=>({v:c,l:c.charAt(0).toUpperCase()+c.slice(1)}))} />
        <AInput  label="Emoji (si pas d'image)" value={form.emoji} onChange={v => s('emoji',v)} />
      ...
      <label style={{ display:'flex', alignItems:'center', gap:8, cursor:'pointer', fontSize:'.88rem', marginTop:'.5rem' }}>
...
function EventsSection() {
  ...
  return (
    <div className={styles.card}>
      <div className={styles.cardHeader}><span className={styles.cardTitle}>🎌 Événements ({events.length})</span></div>
      <div style={{ overflowX:'auto' }}>
        <table className={styles.table}>
      ...
      <div>
        <div className={styles.card} style={{ marginBottom:'1.5rem' }}>
          <div className={styles.cardHeader}>
            <span className={styles.cardTitle}>📝 Articles & Annonces ({posts.length})</span>
          ...
          <div style={{ padding:'1rem' }}>
            ...
            {posts.map(p => (
              <div key={p.id} className={styles.postItem}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontSize:'.9rem', fontWeight:700, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', marginBottom:3 }}>{p.title}</div>
                  <div style={{ display:'flex', gap:7, flexWrap:'wrap', alignItems:'center' }}>
                    <Badge variant={CAT_COLORS[p.category]||'gray'} style={{ fontSize:'.62rem' }}>{p.category}</Badge>
                    <span style={{ fontSize:'.72rem', color:'var(--muted)' }}>{new Date(p.createdAt).toLocaleDateString('fr-FR')}</span>
                    <span style={{ fontSize:'.72rem', color:p.isPublished?'var(--green)':'var(--red)' }}>{p.isPublished?'✅ Publié':'🔴 Masqué'}</span>
                  </div>
                </div>
            ...
            {!posts.length && !lP && <EmptyState icon="📝" title="Aucun article publié" />}
          </div>
        </div>
        <div className={styles.card}>
          <div className={styles.cardHeader}>
          ...
          <div style={{ padding:'1rem', display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))', gap:10 }}>
            {partners.map(p => (
              <div key={p.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px', background:'rgba(255,255,255,.03)', border:'1px solid var(--border)', borderRadius:10 }}>
                <span style={{ fontSize:'1.5rem', width:36, textAlign:'center' }}>{p.logo||'🤝'}</span>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontSize:'.85rem', fontWeight:700, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{p.name}</div>
                  <div style={{ fontSize:'.72rem', color:'var(--muted)' }}>{p.description||''}</div>
                </div>
            ...
            {!partners.length && !lPart && <p style={{ color:'var(--muted)', fontSize:'.85rem' }}>Aucun partenaire</p>}
          </div>
        </div>
      </div>
...
function PostModal({ post:p, onClose, onSave }) {
  ...
  return (
    <Modal isOpen title={p?'✏️ Modifier':'📝 Nouvel article'} onClose={onClose} wide
      ...
      <div className={styles.formGrid2}>
        <ASelect label="Catégorie" value={form.category} onChange={v=>s('category',v)} options={[{v:'blog',l:'📝 Blog'},{v:'event',l:'🎌 Événement'},{v:'promo',l:'🔥 Promo'},{v:'partner',l:'🤝 Partenaire'}]} />
      ...
      <div style={{ display:'flex', gap:'1.5rem', marginTop:'.8rem' }}>
        <label style={{ display:'flex', alignItems:'center', gap:8, cursor:'pointer', fontSize:'.88rem' }}><input type="checkbox" checked={form.isFeatured} onChange={e=>s('isFeatured',e.target.checked)} style={{ accentColor:'var(--green)' }} /> ⭐ À la une</label>
        <label style={{ display:'flex', alignItems:'center', gap:8, cursor:'pointer', fontSize:'.88rem' }}><input type="checkbox" checked={form.isPublished} onChange={e=>s('isPublished',e.target.checked)} style={{ accentColor:'var(--green)' }} /> ✅ Publié</label>
      </div>
    </Modal>
  )
}
...
function PopupForm({ toast }) {
  ...
  return (
    <div style={{ padding:'1rem' }}>
      <p style={{ fontSize:'.8rem', color:'var(--muted)', lineHeight:1.5, marginBottom:'1rem' }}>Popup visible sur la page Blog après quelques secondes.</p>
      <AInput label="Emoji" value={form.emoji} onChange={v=>s('emoji',v)} />
      <AInput label="Titre" value={form.title} onChange={v=>s('title',v)} />
      ...
      <Button variant="primary" onClick={async()=>{ try{ await blogApi.savePopup({...form,isActive:true}); toast.success('✅ Popup activée !') }catch(e){ toast.error(e.message) }}} style={{ width:'100%', marginTop:'.8rem' }}>⚡ Activer</Button>
    </div>
  )
}
...
function UsersSection({ toast }) {
  const [filter,setFilter]=useState('all'); const [search,setSearch]=useState(''); const [selected,setSelected]=useState(null)
  ...
  return (
    <div>
      <div className={styles.filters}>
        ...
        <input className={styles.searchBox} placeholder="🔍 Pseudo ou email..." value={search} onChange={e=>setSearch(e.target.value)} />
      </div>
      <div className={styles.card}>
        <div className={styles.cardHeader}><span className={styles.cardTitle}>👥 Membres ({filtered.length})</span></div>
        <div style={{ overflowX:'auto' }}>
          <table className={styles.table}>
            <thead><tr><th>Membre</th><th>Email</th><th>Ville / Quartier</th><th>Rôle</th><th>Inscrit</th><th>Statut</th><th></th></tr></thead>
            <tbody>
              {filtered.map(u=>(
                <tr key={u.id} className={styles.tr}>
                  <td><strong>{u.pseudo}</strong>{u.firstName&&<div style={{ fontSize:'.75rem', color:'var(--muted)' }}>{u.firstName} {u.lastName||''}</div>}</td>
                  <td style={{ fontSize:'.82rem' }}>{u.email}</td>
                  <td style={{ fontSize:'.82rem' }}>{u.city||'—'}{u.quartier&&<div style={{ fontSize:'.72rem', color:'var(--muted)' }}>📍 {u.quartier}</div>}</td>
                  <td><Badge variant={u.role==='superadmin'?'red':u.role==='admin'?'amber':'gray'} style={{ fontSize:'.65rem' }}>{u.role}</Badge></td>
                  <td style={{ fontSize:'.78rem' }}>{new Date(u.createdAt).toLocaleDateString('fr-FR')}</td>
...
function UserModal({ user:u, onClose, onSave }) {
  const [role,setRole]=useState(u.role); const [ver,setVer]=useState(u.isVerified); const [nl,setNl]=useState(u.newsletterSubscribed)
  return (
    <Modal isOpen title={`👤 ${u.pseudo}`} onClose={onClose} wide
      footer={<><Button variant="ghost" onClick={onClose}>Fermer</Button><Button variant={u.isBanned?'primary':'danger'} onClick={()=>onSave(u.id,{isBanned:!u.isBanned})}>{u.isBanned?'🔓 Débannir':'🔒 Bannir'}</Button><Button variant="primary" onClick={()=>onSave(u.id,{role,isVerified:ver,newsletterSubscribed:nl})}>💾 Sauvegarder</Button></>}>
      <div className={styles.detailGrid} style={{ marginBottom:'1.2rem' }}>
        {[['Pseudo',u.pseudo],['Email',u.email],['Prénom/Nom',`${u.firstName||'—'} ${u.lastName||''}`],['Téléphone',u.phone||'—'],['WhatsApp',u.whatsapp||'—'],['Ville',u.city||'—'],['Quartier',u.quartier||'—'],['Connexions',`${u.loginCount||0}x`]].map(([l,v])=>(
          <div key={l} className={styles.detailItem}><div className={styles.detailLbl}>{l}</div><strong style={{ fontSize:'.88rem' }}>{v}</strong></div>
        ))}
      </div>
      <div style={{ background:'rgba(255,255,255,.02)', border:'1px solid var(--border)', borderRadius:12, padding:'1.2rem' }}>
        <div className={styles.formGrid2}>
          <ASelect label="Rôle" value={role} onChange={setRole} options={[{v:'user',l:'👤 Membre'},{v:'admin',l:'⚙️ Admin'},{v:'superadmin',l:'👑 Super Admin'}]} />
          <ASelect label="Vérification" value={String(ver)} onChange={v=>setVer(v==='true')} options={[{v:'true',l:'✅ Vérifié'},{v:'false',l:'⏳ Non vérifié'}]} />
        </div>
        <ASelect label="Newsletter" value={String(nl)} onChange={v=>setNl(v==='true')} options={[{v:'true',l:'✅ Abonné'},{v:'false',l:'❌ Désabonné'}]} />
      </div>
    </Modal>
  )
}
...
function AInput({ label, type='text', value, onChange, placeholder }) {
  return (
    <div style={{ marginBottom:'1rem' }}>
      <label style={{ display:'block', fontSize:'.7rem', fontWeight:700, letterSpacing:1, color:'var(--muted)', marginBottom:5, textTransform:'uppercase' }}>{label}</label>
      <input type={type} value={value??''} onChange={e=>onChange(e.target.value)} placeholder={placeholder}
        ...
        onFocus={e=>e.target.style.borderColor='var(--green)'} onBlur={e=>e.target.style.borderColor='rgba(255,255,255,.1)'} />
    </div>
  )
}
  ...
  return (
    <div style={{ marginBottom:'1rem' }}>
      ...
      <textarea value={value??''} onChange={e=>onChange(e.target.value)} rows={rows}
        style={{ width:'100%', padding:'10px 12px', borderRadius:10, background:'rgba(255,255,255,.04)', border:'1px solid rgba(255,255,255,.1)', color:'var(--text)', fontFamily:'var(--font-body)', fontSize:'.9rem', outline:'none', resize:'vertical', lineHeight:1.6 }}
        onFocus={e=>e.target.style.borderColor='var(--green)'} onBlur={e=>e.target.style.borderColor='rgba(255,255,255,.1)'} />
    </div>
  )
}